...
function SubjectDetailsModal({ subject, isOpen, onClose }: SubjectDetailsModalProps) {
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium border bg-purple-50 text-purple-700 border-purple-200">
          ...
          {subject.moderation_comment && (
            <div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <p className="text-gray-700">{subject.moderation_comment}</p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
            <div>
              ...
              <p className="text-sm text-gray-900">{formatDateTime(subject.created_at)}</p>
            </div>
            <div>
              <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Last Updated</h4>
              <p className="text-sm text-gray-900">{formatDateTime(subject.updated_at)}</p>
            </div>
            <div>
              <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Created By</h4>
              <p className="text-sm text-gray-900">Teacher ID: {subject.created_by}</p>
            </div>
            <div>
              <h4 className="text-xs font-semibold text-gray-500 uppercase mb-1">Subject ID</h4>
              <p className="text-sm text-gray-900">#{subject.id}</p>
            </div>
          </div>
        </div>
...
export default function SubjectsPage() {
  ...
  const filteredSubjects = useMemo(() => {
    return subjects.filter((subject) => {
      ...
      const matchesStatus =
        selectedStatus === "All" || subject.status.toUpperCase() === selectedStatus.toUpperCase();

      return matchesSearch && matchesGrade && matchesStatus;
    });
  }, [subjects, search, selectedGrade, selectedStatus]);
  ...
  const statuses = ["All", "DRAFT", "PUBLISHED", "PENDING", "REJECTED"];

  const handleView = (subject: TeacherSubject) => {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage and view all your subjects</p>
        </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {pagedSubjects.map((subject) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-700">{subject.grade}</span>
                        </td>
                        <td className="py-4 px-4">
                          <span
                        ...
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-600">{formatDate(subject.created_at)}</span>
                        </td>
                        <td className="py-4 px-4">
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-between flex-col sm:flex-row gap-4">
                  <div className="text-sm text-gray-600">
                    Showing {start + 1} to {Math.min(start + pageSize, filteredSubjects.length)} of{" "}
