...
export default function Navbar({ rightContent, onMenuClick, userRole }: NavbarProps) {
  ...
  const currentTitle = React.useMemo(() => {
    ...
    if (pathname.startsWith("/content/subjects")) return "Subjects";
    if (pathname.startsWith("/content/lessons")) return "Lessons";
    if (pathname.startsWith("/content/games")) return "Games";
    if (pathname.startsWith("/content/teachers")) return "Teachers";
    if (pathname.startsWith("/content/assessments")) return "Assessments";
    if (pathname.startsWith("/content/settings")) return "Settings";
    return "";
  }, [pathname]);
  return (
    <header className="fixed top-0 left-0 z-50 h-16 w-full border-b border-black/5 bg-white/90 backdrop-blur supports-backdrop-filter:bg-white/60">
      <div className="flex h-full w-full items-center justify-between px-4 sm:px-6 lg:px-8 sm:pl-64">
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          {!isValidator && pathname?.startsWith("/content/subjects") ? (
            <Link href="/content/subjects/create" className="hidden sm:inline-flex items-center gap-2 rounded-full bg-emerald-600 px-5 py-2.5 text-white shadow hover:bg-emerald-700">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Create Subject
            </Link>
          ) : null}
          {!isValidator && pathname?.startsWith("/content/lessons") ? (
            <Link href="/content/lessons/create" className="hidden sm:inline-flex items-center gap-2 rounded-full bg-emerald-600 px-5 py-2.5 text-white shadow hover:bg-emerald-700">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Create Lesson
            </Link>
          ) : null}
          ...
          <button
            ...
            className="hidden md:inline-flex relative items-center justify-center rounded-md p-2 text-gray-700 hover:bg-gray-100"
          >
